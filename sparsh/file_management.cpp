...
void displayLeaderboard() {
    while (true) {
        ...
        // Display leaderboard in tabular format
        renderText(200, 100, "Rank");       // Column headers
        renderText(400, 100, "Name");
        renderText(600, 100, "Score");

        int y = 200; // Start position for rows
        int rank = 1; // Rank counter
        for (const auto& entry : leaderboard) {
            size_t separatorPos = entry.find(','); // Split entry into name and score
            if (separatorPos != std::string::npos) {
                std::string name = entry.substr(0, separatorPos);
                std::string score = entry.substr(separatorPos + 1);

                renderText(200, y, std::to_string(rank).c_str());      // Name column
                renderText(400, y, name.c_str());     // Score column
                renderText(600, y, score.c_str()); // Rank column
            } else {
                renderText(200, y, entry.c_str());     // Handle malformed entries
            }

            y += 50; // Move to the next row
            rank++;   // Increment rank
        }

        renderText(100, y, "Press ESC to return to the Main Menu.");
